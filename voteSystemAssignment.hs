...
zipCandiates candidates votePref = trace ("\n")zip (drop 2 candidates) (drop 2 votePref)


--formatVotes applies zipCandiates to the dirtyVotes list formatting them like:
--[("D. Abbott", 1), ("E. Balls", 2), ("A. Burbhm", 3), ("D. Milliband", 4), ("E. Milliband", 5)]
formatVotes :: [[(String, String)]]
formatVotes = map (zipCandiates (head dirtyVotes)) (drop 1 dirtyVotes)


--Insertion sort which recursively sorts based on the second value in the tuple
insert :: Ord a => (String, a) -> [(String, a)] -> [(String, a)]
insert x [] = [x]
insert x (y:ys)
          | snd x <= snd y = x : y : ys
...
--sortVotes applies the insertion sort functions to the output of formatVotes
sortVotes :: [[(String, String)]]
sortVotes = map (isort) formatVotes


--removeBlankVotes gets rid of any instance where there is an empty
--string or "*" inplace of a preference
removeBlankVotes :: [(String, String)] -> [String]
removeBlankVotes xs = [fst x | x <- xs, snd x/="*", snd x/=""]


--cleanVotes applies removeBlankVotes to the output of sortVotes
